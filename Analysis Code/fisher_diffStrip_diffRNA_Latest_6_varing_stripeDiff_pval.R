...
bar_pval_df_c<-c("depth","res","pval","data","observ","expect","fold")

for(depth in depth_list){
# foreach::foreach(depth=depth_list,.combine=cbind)%dopar%{
  for (pval in c(0.05,0.01,0.001)){
  # foreach::foreach(pval=c(0.05,0.04,0.03,0.02,0.01),.combine=cbind)%dopar%{
    folder=paste(home,"/1_24/data_Latest_6_varingStripeDiff",depth,"_",tt,"_",pval,sep="")
    folder2=paste(home,"/1_24/data","_",pval,"_",tt,sep="")
    ...
    # foreach::foreach(binSize=binSizeListt,.combine=cbind)%dopar%{
      ...
      print(paste(res,pval,depth,res,sep="_"))
      dataName=paste("A_6_varingStripeDiff_stripe_",type_list[1],"_",type_list[2],"_",pval,sep="")
      ...
      dataName2=paste("A_6_varingStripeDiff_FinalResult_",type_list[1],"_",type_list[2],"_",pval,sep="")
      ...
      if(!file.exists(paste(folder,"/","CM_",res,"_",pval,depth,"Fisher.rds",sep=""))){
        for(id in id_list){
        ...
        cm[cm==0]=1
        saveRDS(cm,paste(folder,"/","CM_",res,"_",pval,depth,".rds",sep=""))
        FET=fisher.test(cm,simulate.p.value=TRUE,B=1e7)$p.value
        saveRDS(FET,paste(folder,"/","CM_",res,"_",pval,depth,"Fisher.rds",sep=""))
      }
      cm=readRDS(paste(folder,"/","CM_",res,"_",pval,depth,".rds",sep=""))
      FET=readRDS(paste(folder,"/","CM_",res,"_",pval,depth,"Fisher.rds",sep=""))
      df=c()
      ...
      IMR90_FET=FET
      saveRDS(FET,paste(folder,"/","CM_",res,"_",pval,depth,"Fisher_IMR90.rds",sep=""))
      saveRDS(IMR90,paste(folder,"/","CM_",res,"_",pval,depth,"IMR90.rds",sep=""))
      df=c()
      ...
      HUVEC_FET=FET
      saveRDS(FET,paste(folder,"/","CM_",res,"_",pval,depth,"Fisher_HUVEC.rds",sep=""))
      saveRDS(HUVEC,paste(folder,"/","CM_",res,"_",pval,depth,"HUVEC.rds",sep=""))
      df=c()
      ...
      bar_pval_df=rbind(bar_pval_df,c(strsplit(depth,"_")[[1]][2],res,pval,"HUVEC",bar_df[1,"percentage"],bar_df[2,"percentage"],round(bar_df[1,"percentage"]/bar_df[2,"percentage"],2)))
      bar_pval_df=rbind(bar_pval_df,c(strsplit(depth,"_")[[1]][2],res,pval,"IMR90",bar_df[3,"percentage"],bar_df[4,"percentage"],round(bar_df[3,"percentage"]/bar_df[4,"percentage"],2)))
      saveRDS(bar_pval_df,paste(folder,"/../confusionPlot_6_varingStripeDiff_HUVEC_IMR90.rds",sep=""))
...
bar_pval_df$pval=factor(bar_pval_df$pval,c(0.05,0.01,0.001))
# bar_pval_df1 <- arrange(bar_pval_df1,data,normMethod)
bar_pval_df1 <- ddply(bar_pval_df1, "pval",transform, label_ypos=cumsum(fold))
head(bar_pval_df1)
print(ggplot(bar_pval_df1, aes(x=pval, y=fold, fill=data)) + geom_bar(stat="identity")+
        theme_classic()+ 
...
ggsave(filename=paste(folder,"/../","barplot_","confusionPlot_6_varingStripeDiff_HUVEC_IMR90.svg",sep=""),dpi = 300,width=3,height=4)
print(dev.off())
###########################################
